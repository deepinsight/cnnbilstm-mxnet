...
#A version that fully preload data.
...
class OCRIter(mx.io.DataIter):
    ...
    def __iter__(self):
        ...
        print "All {0} Label Processed.".format(pic_num)
        
        for k in range(pic_num / self.batch_size ):        
            data_all = [mx.nd.array(data[(k*self.batch_size):((k+1)*self.batch_size)])] + self.init_state_arrays
            label_all = [mx.nd.array(label[(k*self.batch_size):((k+1)*self.batch_size)])]
            data_names = ['data'] + init_state_names
            label_names = ['label']
...
if __name__ == '__main__':
    ...
    # 1024*2, WIDTH=900, SEQ=60, lr = 0.0005 Best Ever
    
    prefix = 'mymodel/LANGUAGE_BATCH128_SEQ60_varlen_1024-2-0.0005-50-mmt0.9'
    iteration = 10
    ...
    train_path  = './img_data/train_data_varlen_5_45_CHLang/'
    test_path  = './img_data/test_data_varlen_5_45_CHLang/'
