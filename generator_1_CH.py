...
def captcha_draw_fixedstring(size_im, fixstring, 
    ...
    width_cha = int(width_im / max ( num_han+num_nonhan*NONHAN_SHRINKAGE - overlap, 1)) # 字符区域宽度
    height_cha = height_im*1.0# 字符区域高度
    ...
    for i, cha in enumerate(fixstring):
        ...
        if rd_text_pos:
            ...
            dery = random.randint(0, max(int(height_cha)-size_cha-5, 0))
        ...
        if cha == '.':
            ...
            im.paste(im_cha, (int(max(i, 0)*width_cha 
                                  - totalshrinkedcount*width_cha*(1-NONHAN_SHRINKAGE)) + derx + 2, dery + 23), im_cha) # 字符左上角位置
            contents.append(cha)
            ...
            im.paste(im_cha, (int(max(i, 0)*width_cha - totalshrinkedcount*width_cha*(1-NONHAN_SHRINKAGE))+derx -1, dery -2 )) # 字符左上角位置
            contents.append(cha)
            #print size_cha,           
...
def test_en_ch():
    ...
    for i in range(0,nb_image):
        ...
        fixstring , _ = gen_fixlen(fixlen=6, hanlenmax=1, bkfix = 0, bkmax = 0,
                                  fltlen = 1, prefixmax= 1, suffixmax = 1,
                                  log10nummax = 2)
        ...
        if num_pic % 1001 == 0:
            dir_folder += 1
        dir_name = 'train_data_1_CHLang'
        dir_path = '/data1/img_data/'+dir_name+'/'
        #print fixstring
        captcha_draw_fixedstring(size_im=size_im, fixstring=fixstring[1],
                ...
                rotate=rotate, dir_path=dir_path, fonts=font_path,img_num = nb_image,img_now = random.randint(0,10000000))
