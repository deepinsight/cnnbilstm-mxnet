...
def captcha_draw_fixedstring(size_im, fixstring, 
    ...
    for i, cha in enumerate(fixstring):
        ...
        if rd_text_color:
            text_color = randRGB()
        if rd_text_pos:
            derx = random.randint(0, max(width_cha-size_cha-20, 0))
            if not chaIsHan:
                derx = int((derx+random.randint(0,1))/2)
            dery = random.randint(0, max(int(height_cha)-size_cha-20, 0))
...
def test_en_ch():
    ...
    for i in range(0,nb_image):
        ...
        if num_pic % 1001 == 0:
            dir_folder += 1
        dir_name = 'train_data_4_CHLang'
        dir_path = '/data1/img_data/'+dir_name+'/'
        #print fixstring
        captcha_draw_fixedstring(size_im=size_im, fixstring=fixstring[:4],
